...
def validate_no_vector_db_settings() -> None:
    ...
    if ENABLE_CRAFT:
        raise RuntimeError(
            ...
            "Insight Craft requires background workers for sandbox lifecycle "
...
def get_application(lifespan_override: Lifespan | None = None) -> FastAPI:
    application = FastAPI(
        ...
        version=__version__,
        description="Insight API for AI-powered chat with search, document indexing, agents, actions, and more",
        servers=[
            {"url": f"{WEB_DOMAIN.rstrip('/')}/api", "description": "Insight API Server"}
        ],
...
if __name__ == "__main__":
    logger.notice(
        f"Starting Insight Backend version {__version__} on http://{APP_HOST}:{str(APP_PORT)}/"
    )
